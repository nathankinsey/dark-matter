...
#SBATCH --array=1
#SBATCH --output=slurm_out/dm_extr_ann%j.log   # Standard output and error log
pwd; hostname; date

rawdata="/share/oberbaurlab/LCG/Dog_Genome_Consortium/dogs.648.vars.ann.vcf.gz"
outpath="/share/oberbaurlab/nk/DarkMatter/ChrVCFs"
...
java -Xmx8g -jar snpEff.jar CanFam3.1.101 ${outpath}/dbvdc645_${SLURM_ARRAY_TASK_ID}.vcf > ${outpath}/dbvdc645ann_${SLURM_ARRAY_TASK_ID}.vcf
rm ${outpath}/dbvdc645_${SLURM_ARRAY_TASK_ID}.vcf

# Filter out all variants that don't have a high or moderate calculated impact on translation
java -Xmx8g -jar SnpSift.jar filter "ANN[*].IMPACT has 'HIGH' | ANN[*].IMPACT has 'MODERATE'" ${outpath}/dbvdc645ann_${SLURM_ARRAY_TASK_ID}.vcf > ${outpath}/dbvdc645exome_${SLURM_ARRAY_TASK_ID}.vcf
module unload java/jdk14.0.2

# Compress the VCF files and remove the headers from the "exome" data for easier processing in R. Delete the uncompressed files
cd $outpath
module load bcftools/1.10.2
bcftools view -l 9 -O z -o dbvdc645ann_${SLURM_ARRAY_TASK_ID}.vcf.gz --threads 6 dbvdc645ann_${SLURM_ARRAY_TASK_ID}.vcf
bcftools view -H -l 9 -O z -o dbvdc645exome_${SLURM_ARRAY_TASK_ID}.vcf.gz --threads 6 dbvdc645exome_${SLURM_ARRAY_TASK_ID}.vcf
```
